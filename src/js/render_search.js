...
const paginationListRef = document.querySelector('.pagination .pagination-list');
...
paginationListRef.addEventListener('click', (event) => {
	if (event.target.parentNode.classList.contains('pagination-number') && !event.target.parentNode.classList.contains('active')) {
...
function handlePageChange(currentPage) {
		...
		moviesApiService.getFilmsByName().then(({ data }) => {
			clearMoviesContainer();
			renderMovieList(data.results.slice(0, 3));
